...
DTICK_MAP       = {"1D": 0.25, "1W": 0.25, "1M": 0.5,   "3M": 1.0,   "6M": 1.0,   "1Y": 2.0}
...
if view == "Invested positions only":
    ...
    fig.update_layout(yaxis=dict(title="Return on invested capital (%)", tickformat="+.2f", ticksuffix="%", dtick=dtick, exponentformat="none"))
    ...
    _add_benchmark("SPY", "SPY (S&P 500)",   "#4A90D9")
    _add_benchmark("VTI", "VTI (Total Mkt)", "#F5A623")
    fig.update_layout(yaxis=dict(title="Total account return (%)", tickformat="+.2f", ticksuffix="%", dtick=dtick, exponentformat="none"))
